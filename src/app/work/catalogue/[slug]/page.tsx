...
export async function generateMetadata({
  ...
  return {
    ...
    openGraph: {
      ...
      type: "article",
      publishedTime,
      url: `https://tom.so/work/catalogue/${post.slug}`,
      images: [
        {
          url: ogImage,
          width: 1200,
    ...
    <>
      ...
      <div className="flex justify-between items-center mt-2 mb-8 text-sm max-w-[650px]">
        <p className="text-sm text-neutral-700 dark:text-neutral-200">
          Made on {formatDate(post.metadata.publishedAt)}
        </p>
      </div>
...
export default async function Catalogue({ params }: Readonly<PageParams>) {
  const post = await getPostBySlug(params.slug);
